...
export default function DashboardBody(props) {
    ...
    useEffect(()=> {
        Axios.get(`http://localhost:3003/products/${props.productId}`)
            .then(res => {
                setProduct(res.data)
            } 
        )
        Axios.get(`http://localhost:3003/news/${props.productId}`)
            .then(res => {
                setNews(res.data)
            } 
        )
        Axios.get(`http://localhost:3003/users/list/${props.productId}`)
            .then(res => {
                setUser(res.data)
            } 
        )
        Axios.get(`http://localhost:3003/order/${props.productId}`)
            .then(res => {
                setOrder(res.data)
            } 
        )
        Axios.get(`http://localhost:3003/collection/${props.productId}`)
            .then(res => {
                setCollection(res.data)
            } 
        )
        Axios.get(`http://localhost:3003/email/${props.productId}`)
            .then(res => {
                setEmail(res.data)
            } 
        )
