...
export default function DashboardProductEdit(props) {
    ...
    const onSubmit = (event) => {
        ...
        axios.post(`http://localhost:3003/products/update/${product._id}`, formData, config)
        .then(()=>{
    ...
    const addNewCate = () => {
        axios.post('http://localhost:3003/category', {
    ...
    const deleteImg = (event) => {
        ...
        items.splice(id, 1)
        setProductImg(items)
        axios.post(`http://localhost:3003/products/update/${product._id}`, {
            deleteImgId: id
        })
    }
