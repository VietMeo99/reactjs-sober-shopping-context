...
export default function DashboardOrderCreate(props) {
    ...
    useEffect(()=>{
        if (chooseUser === false) {
            axios.get(`http://localhost:3003/vietnam`)
                .then(res => {
                    ...
                    if (order) {
                        ...
                        if(typeof order.orderList !== "undefined") {
                            order.orderList.map((item)=>{
                                axios.get(`http://localhost:3003/products/${item.id}`)
                                    .then(res => {
        ...
        axios.get(`http://localhost:3003/users/list`)
            .then(res => {
    ...
    const onSubmit = (event) => {
        ...
        axios.post(`http://localhost:3003/order/update/${order._id}`, {
            orderName: orderName,
